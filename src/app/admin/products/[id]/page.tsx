...
import { db, Product, ColorVariant } from "@/lib/instant";
...
export default function EditProduct() {
  ...
  useEffect(() => {
    if (product && !form) {
      setForm({
        ...
        badge: (product.badge as ProductForm["badge"]) || "",
        image: product.image || "",
        colors: product.colors ? JSON.parse(product.colors) : [],
        sizes: product.sizes ? JSON.parse(product.sizes) : [],
        features: product.features ? JSON.parse(product.features) : [],
        careInstructions: product.careInstructions ? JSON.parse(product.careInstructions) : [],
  ...
  const addFeature = () => {
    if (!form || !newFeature.trim()) return;
  ...
  const addCareInstruction = () => {
    if (!form || !newCareInstruction.trim()) return;
  ...
  const saveProduct = async () => {
    if (!form || !form.name || !form.price || !form.image) {
    ...
    return (
      <div className="min-h-screen bg-stone-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/admin/products" className="text-stone-900 underline">
            Back to products
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-stone-500 mt-1">{form.name || "Untitled"}</p>
          </div>
          <Link
            href="/admin/products"
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm border border-stone-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-stone-500 mt-1">
                  {form.published
                    ? "Product is visible on your store"
