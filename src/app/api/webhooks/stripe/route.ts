  ...
  if (event.type === "checkout.session.completed") {
    ...
    try {
      ...
      const orderItems = fullSession.line_items?.data.map((item) => {
        ...
        const productWithMeta = product as Stripe.Product & { metadata?: Record<string, string> };

        // Extract color and size from product metadata (set during checkout)
        const color = productWithMeta?.metadata?.color || "";
        const size = productWithMeta?.metadata?.size || "";

        // Build full name for fulfillment parsing
        let fullName = product?.name || item.description || "Unknown Product";

        // If color/size are in metadata but not in product name, append them
        if ((color || size) && !fullName.includes(color)) {
          fullName = color && size
            ? `${fullName} - ${color} - ${size}`
            : color
            ? `${fullName} - ${color}`
      ...
      console.log(`[Fulfillment Debug] Order items:`, JSON.stringify(orderItems));
